...
Elyra Canvas inlines SVG files as-is and performs no style transformation at runtime. SVG files that contain `<style>` blocks or `style=` attributes will cause CSP violations when the application enforces `style-src` without `'unsafe-inline'`.
