...
#### **enableLinkMethod**
This can be: "Ports" or "Freeform". The default is "Ports". When set to "Ports", each link will be drawn from a specific output port on the source node to a specific input port on the target node. Note: With this option, links will be drawn to the port locations even if the port display has been switched off by the application. When set to "Freeform" links will be drawn from the source node to the target node ignoring where ports have been positioned. Applications displaying freeform links usually choose not to display input and output ports.
...
!!! warning "enableStraightLinksAsFreeform - deprecated"
    ...
    This field is deprecated and will be removed in the next major Elyra Canvas version so it is recommended that applications using "Straight" links explicitly set `enableLinkMethod` to "Freeform" now because that will be mandatory when migrating to the next version.
...
#### **enablePaletteHeader**
This is a JSX object that will displayed in an open, wide palette. It is positioned below the Search bar and above the categories and nodes. The default is null, which means nothing will be displayed. This option can be used by the application to add application specific function into the palette, for example, a button could be added.
