...
The notification panel is displayed by the user by clicking the notifications icon in the toolbar. Your application can display whatever messages it wants in the notification panel. See the [Notification Messages](03.04.05-notification-messages.md) page for the structure of message objects. The contents of the notification panel can be managed using the methods below:
```js
...
!!! note "Content Security Policy"
    The `style` specification passed to `setObjectsStyle` and `setObjectsMultiStyle` is
    rendered as an inline `style` attribute directly on the SVG node and comment elements.
    Applications that use these methods must include `'unsafe-inline'` in their
    `Content-Security-Policy` `style-src` directive. Applications that do not use these
    methods can omit `'unsafe-inline'` from `style-src`.
...
!!! note "Content Security Policy"
    The `style` specification passed to `setLinksStyle` and `setLinksMultiStyle` is
    rendered as an inline `style` attribute directly on the SVG link elements.
    Applications that use these methods must include `'unsafe-inline'` in their
    `Content-Security-Policy` `style-src` directive. Applications that do not use these
    methods can omit `'unsafe-inline'` from `style-src`.
...
setRightFlyoutWidth(width)

// Sets the minimum width for the right-flyout
 setRightFlyoutMinWidth(width)
```
...
Focus highlighting is typically shown by a blue outline box and can be moved around Common Canvas objects using various [keyboard shortcuts](03.05-keyboard-support.md). The focus can be managed using the following methods:
