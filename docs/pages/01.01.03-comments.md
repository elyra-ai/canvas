...
Optionally, the application can switch on [markdown support](03.02.01-canvas-config.md/#enablemarkdownincomments) within comments. This allows the user to enter markdown syntax when the comment is in edit mode, which is then formatted appropriately in display mode.
...
Comments support the same decoration system as nodes. Decorations (images, labels, shapes, or JSX objects) can be added to a comment programmatically using the [CanvasController decoration API](03.04.01-decorations.md). Decoration positions use the same 9-point anchor system (`topLeft`, `topCenter`, … `bottomRight`) as node decorations. Decorations stored in the pipelineFlow document are persisted under `app_data.ui_data.decorations` on the comment object.

See the [Decorations](01.01.04-decorations.md) page for a general introduction and the [CanvasController API documentation](03.04-canvas-controller.md) for details of the comment decoration methods.
