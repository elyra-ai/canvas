...
When more than one link enters or exits into/from one side of the node the freeform links will be drawn to maintain some separation like this:
...
When links are specified to be drawn between ports there are 4 possible choices for [enableLinkDirection](03.02.01-canvas-config.md/#enablelinkdirection): "LeftRight", "RightLeft", TopBottom" or BottomTop". With these settings, common canvas will draw the link to correctly show the flow from output ports to input ports regardless of where the nodes are positioned relative to one another.

Here is an example of the "LeftRight" links direction:
...
Links between ports can be useful if the application want to allow the user to create loops in the flow, like this:

<img src="../assets/cc-link-ports-elbow-loop.png" width="850" />

#### Overriding port position and direction

The four 'ports' options for enableLinkDirection will control the default position of the input and otput ports as described above however, the application can customize the position of the ports on the node and the position of the ports will affect the way [links are drawn from/to the port](03.06.01-node-customization.md/#overriding-port-position-and-link-direction).
...
With this option, the start and end of links can be dragged away from the nodes they are connected to and dropped onto an arbitrary point on the canvas to create a 'detached' link. Alternatively, links that originate from, or point to, a point on the canvas can be dragged to nodes to form a connection. Also, nodes can be dragged to the ends of those 'detached' links to form a connection.
