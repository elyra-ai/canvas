...
If nodes are set to be resizable ([enableResizableNodes](03.02.01-canvas-config.md#enableresizablenodes) set to `true`) and the nodes is resized it will look like this, because the second port is tied to the "bottomLeft" position:
...
externalUtils - an additional utilities file
```
