...
A super node is a special kind of node that can reference another pipeline in the pipeline flow object or, in some cases, in another (external) pipeline flow. The referenced pipeline is known as a sub-flow or sub-pipeline.
...
The supernode can be edited by the user, after it has been created, by adding, removing or editing its nodes, comments and links.
...
* Converted from internal to external - this means the sub-flow is removed from its current pipeline flow and placed in a separate pipeline flow object that can be saved by the application. This is useful if the sub-flow contains a set of common steps that need to be reused (referenced) by other pipeline flows.
* Converted from an external to internal - this means the sub-flow is copied from the external pipeline flow and places into the pipeline flow of the supernode.
