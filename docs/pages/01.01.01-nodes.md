...
* Converted from internal to external - this means the sub-flow is removed from its current pipeline flow and placed in a separate pipeleine flow object that can be saved by the application. This is useful if the sub-flow contains a set of common steps that need to be reused (referenced) by other pipeline flows.
