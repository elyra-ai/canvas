...
Nodes can be resized if the [enableResizableNodes](03.02.01-canvas-config.md/#enableresizablenodes) canvas config field is set to `true`. The node can be resized if the user drags the border area in the desired direction.

<img src="../assets/cc-node-resize.gif" width="150" />

## Insert node to link

Nodes can be inserted into a link and the flow re-wired appropriately, if [enableInsertNodeDroppedOnLink](03.02.01-canvas-config.md/#enableinsertnodedroppedonlink) canvas config field is set to `true`.
