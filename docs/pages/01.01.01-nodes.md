...
* image -- or image location on the server
* ports info (inputs and outputs)
* optional decoration information

Additional information on the node layout is stored in the [node layout fields](03.06.01-node-customization.md/#customizing-layout-fields).
...
There can be multiple connections made into an input port or out of an output port. Each port has a maximum and minimum cardinality values which indicate the limits of connection for the port. Common Canvas ensures the maximum cardinality is not exceeded for port connections.

Ports can be shown visually on, or close to, the node as circles or images. This can be cntrolled by the [node layout fields](03.06.01-node-customization.md/#customizing-layout-fields).
...
A super node is a special kind of node that can reference another pipeline in the pipeline flow object or, in some cases, the referenced pipeline can be in another pipeline flow. The referenced pipeline is also known as a sub-flow.

<img src="../assets/cc-pipeline-flow-supernode-diagram.png" width="800" />
...
The user can also navigate to a 'full-page' view (well, really it is full viewport):

<img src="../assets/cc-node-supernode-full-page.gif" width="700" />

The supernode can be edited by the user after it has been created.
...
* Converted from internal sub-flow to external sub-flow - this means the sub-flow is placed in a separate pipeleine flow object and can be serialized and saved by the application. This is useful if the sub-flow contains a set of common steps that need to be reused (referenced) by other pipeline flows.
* Converted from an external sub-flow to an internal sub-flow.
