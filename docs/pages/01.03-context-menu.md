...
* `Control + click` is done on an object or
...
By default, Common Canvas will display appropriate context menus for all flow editor canvas objects. The application can override or replace the default menus by implementing the [`contextMenuHandler`](03.03.01-context-menu-handler.md) callback.
