...
In Common Canvas, nodes, links, and comments can be enhanced with decorations. A decoration is either:
...
Decorations can be applied to a node, link, or comment in a [number of ways](03.04.01-decorations.md) and can be made interactable so when the user clicks on the decoration the [`decorationActionHandler`](03.03.05-decoration-action-handler.md) is called.
