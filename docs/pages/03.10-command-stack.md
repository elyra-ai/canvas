...
Common Canvas provides one command for each action that can be performed in the [flow editor](01.01-flow-editor.md). The application can optionally add its own commands to the command stack.
...
If the application specifies its own canvas toolbar, or its own context menu/toolbar for the flow editor canvas, it can include definitions for the `undo` and `redo` actions if needed.
...
getLabel() - Returns a label that describes the action.
...
```
   ...
   Here is an example showing how the command action would be created and pushed onto the stack using the canvas-controller:
...
functionality, if necessary. The classes that are exported are:
...
The constructors for these classes all take the same two parameters. The `data` object that describes the command
