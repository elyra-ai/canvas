# Decorations
Your application can add Decorations -- additional icons, text labels, shapes or JSX objects -- to nodes, links, or comments in the canvas to indicate special status or attributes of the object. Decorations can be static (for display only) or interactive (a hotspot) so the user can click them to initiate some action.

## Adding Decorations
Decorations can be added to nodes, links, and/or comments in the following ways:
...
3. Decorations can be applied to, and retrieved from, nodes, links, or comments in the pipelineFlow in the `app_data.ui_data` section for the node, link, or comment. (Note: JSX objects are not supported in the pipelineFlow document).
...
#### **id**
A unique ID for the decoration within the context of the node, link, or comment to which the decorator is attached.
...
#### **position**
This is the anchor point to which the decoration is attached.  For a node or comment, this can be one of these 9 enumerated values:
```
