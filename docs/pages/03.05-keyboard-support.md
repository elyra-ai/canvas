...
Common Canvas supports a number of keyboard interactions as described below. Some keyboard shortcuts are only available if the config field `enableKeyboardNavigation` is set to `true` as indicated above each table.
...
|---|---|---|
|Meta + a |selectAll| Selects all objects|
...
|Meta + Shift + [8 key] |zoomIn| Zoom in the flow editor|
|Meta + Shift + [9 key] |zoomOut| Zoom out the flow editor|
|Meta + Shift + [0 (zero) key] |zoomToFit| Zooms to fit the flow objects within the flow editor viewport|
|Meta + Shift + [up arrow key] |-| Pans the flow objects within the flow editor viewport upwards|
|Meta + Shift + [down arrow key] |-| Pans the flow objects within the flow editor viewport downwards|
|Meta + Shift + [left arrow key] |-| Pans the flow objects within the flow editor viewport to the left|
|Meta + Shift + [right arrow key] |-| Pans the flow objects within the flow editor viewport to the right|
...
|Shift + Alt + [down arrow key] |-| Move focus to first sub-object (port/decoration) in the focused node, link, or comment|
...
### When focus is on a sub-object (port or decoration) of a node, link, or comment in the flow editor

The shortcuts in this table are only available when the canvas config field [enableKeyboardNavigation](03.02.01-canvas-config.md#enablekeyboardnavigation) is set to `true`.

Focus is initially moved to the sub-objects of a node, link, or comment by pressing `Shift + Alt + [down arrow key]`. After that the following short cuts are available.

|Keyboard Shortcut|Action|Description|
|---|---|---|
|[tab key] or Shift + Alt + [down arrow key] |-| Move focus to next sub-object (port/decoration/link-handle) in the focused node, link, or comment|
|Shift + [tab key] or Shift + Alt + [up arrow key] |-| Move focus to previous sub-object (port/decoration/link-handle) in the focused node, link, or comment|
...
|Meta + Shift + [L Key] |-| When focus is on an output port, marks it as the connection source for creating a link. Pressing the shortcut again toggles the marking off.|
|Meta + Shift + [L Key] |-| When focus is on an input port and a source output port is marked, creates a link from the marked source port to the focused input port.|
...
|Meta + Shift + x |-| Insert 'strike through' syntax around the selected text|
...
|Meta + 1 |-| Increases number of hashes in front of the selected text|
|Meta + 2 |-| Decreases number of hashes in front of the selected text|
...
|[left arrow key] |-| When focus is on a button in the toolbar, move focus to the button to the left of current focus position. When focus is on a sub-menu or sub-panel, closes the area and moves focus to the parent menu. |
...
|[right arrow key] |-| Opens cascade sub-area, if there is one, for the currently focused item |
