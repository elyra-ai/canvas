# Flow Editor Customization

The flow editor background can be customized by the application in the following ways.

### Snap to grid

Nodes and comments displayed by Common Canvas can be made to ["snap-to-grid"](01.01-flow-editor.md/#snap-to-grid-positioning). The snap behavior can be either during the drag operation or after the drag operation has ended.  The snap to grid behavior can be set in the [enableSnapToGridType](03.02.01-canvas-config.md/#enablesnaptogridtype) config property.
...
The Flow Editor can display four types of [background grid](01.01-flow-editor.md/#grid-background-patterns) underneath the nodes, links and comments.
...
If the grid type is set to either "DotsAndLines" or BoxesAndLines" the following properties can be used to set the size of the line grid:
...
* a number which will be a distance within the flow editor coordinate system or
* a percentage (for example "25%") which will be the percentage of the default width or height of the node.
