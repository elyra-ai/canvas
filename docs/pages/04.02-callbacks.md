...
```
  const callbacks = {
      ...
      expressionLanguageHandler: this.expressionLanguageHandler
  };
```
...
#### CustomDataTypeIconData
Called to fetch icon for the custom data types used in field picker component.

- data: an object that consists of
    - type: `customDataTypeIcon`
    - buttonId: of the corresponding field - the field name provided in uihints
    - dataType: the type of the field specified in uihints. The corresponding icon is expected to be returned in the callback.

```js
buttonIconHandler(data, callbackIcon) {
  if (data.type === "customDataTypeIcon") {
			let dataIcon = null;
...
```
FieldPicker supports custom labels for custom data types. To define a custom label, use the following format in the locale:
  - fieldPicker.`<type>`.label

If a custom label is defined using this key, it will be displayed in the UI. Else, the raw `<type>` value will be displayed.

Example:
```js
{
  "fieldPicker.customType.label": "Custom Label"
}
```
...
### panelTitleHandler()
Optional callback used to customize the title display of panels. This callback is currently only called when rendering a twisty panel and allows you to replace the default panel title with custom content (such as adding icons, badges, or additional text).

The callback receives an object with `panelId`, `label`, and `data` properties, and should return a string or JSX element to be used as the panel title, or null to use the default label.

**Parameters:**
- `panelId` (string): The ID of the panel
- `label` (string): The default label/title of the panel
- `data` (object, optional): Optional data object associated with the panel, as specified in the parameter definition

```js
panelTitleHandler({ panelId, label, data }) {
  if (panelId === "twisty-panel-id") {
    return (
      <div className="custom-panel-title">
        <span>{label}</span>
        <span className="custom-subtitle">sub title</span>
        {data && data.badge && <span className="badge">{data.badge}</span>}
      </div>
    );
  }
  return null; // Use default label specified in parameter defintion
}
```


### expressionVariablesHandler(propertyId, language)
Optional callback for supplying autocomplete variable completions to an expression
control. This is the recommended approach for applications that use a custom expression
language (e.g. JUEL, SpEL) and want their available variables visible in the editor
without building a custom control.

When this callback is registered it is called on every autocomplete trigger, so the
returned variables always reflect the latest app state. They are appended to the
built-in completion list alongside any `dataset_metadata` field names.

**Parameters:**
- `propertyId` (object): The property ID of the expression control being created.
- `language` (string): The raw `language` value from the control's parameter definition.

**Return value:**
An array of variable completions, or `null` / `[]` to add nothing. Each entry can be
either a plain string (the label) or an object `{ label, type }`.

**Step 1 — Declare the language in your parameter definition**

Set `language` in the `parameter_info` entry for the expression control. This value is
passed to the handler so you can distinguish between multiple expression controls in the
same dialog.

```json
"parameter_info": [{
  "parameter_ref": "myExpression",
  "language": "juel"
}]
```
...
The simplest form — return a static list of variable strings:

```js
expressionVariablesHandler(propertyId, language) {
  if (language === "juel") {
    // Plain strings are treated as completion items of type "variable".
    return ["${customer.name}", "${customer.age}", "${order.total}"];
  }
  return []; // no extra completions for other languages
}
```

**Step 3 — Register the callback**

Pass it in the `callbacks` object when rendering `CommonProperties`:
...
**Step 4 (optional) — Supply runtime variables from your pipeline context**

If your variables depend on which node is open (e.g. upstream outputs in a pipeline),
pass the node context via `propertiesInfo.appData` when opening the dialog and read it
back inside the handler using `this.propertiesController.getAppData()`.
`this.propertiesController` is the instance supplied by the `propertiesControllerHandler`
callback — store it on your class when that callback fires.

Because the handler is called on every autocomplete trigger, it will always see the
current `appData` — no re-mounting or special wiring is required.

```js
// When opening the node properties dialog, attach runtime context to appData:
const propsInfo = {
  parameterDef: nodeProperties,
  appData: {
    nodeId: node.id,
    pipelineId: activePipelineId
  }
};
this.setState({ propertiesInfo: propsInfo });

// Inside the handler, retrieve that context and query your own pipeline model:
expressionVariablesHandler(propertyId, language) {
  if (language === "juel") {
    const { nodeId, pipelineId } = this.propertiesController.getAppData() || {};
    if (nodeId) {
      // Query your own pipeline model for upstream variables here.
      // e.g. return myPipelineModel.getVariablesUpstreamOf(nodeId, pipelineId);
    }
  }
  return [];
}
...
`appData` is a general-purpose slot — put whatever context your handler needs in it.

---

### expressionLanguageHandler(propertyId, language)
**Optional.** Callback for supplying **custom syntax highlighting** to an expression control
whose `language` value is not natively supported (e.g. JUEL, SpEL, or any
domain-specific expression language). Use this only when you need to tokenize
and colour your language's syntax - for variable autocomplete alone, use
`expressionVariablesHandler` above.

> **Note:** This callback is not utilized in the harness. It needs a direct dependency
> on `@codemirror/language` in your own application. That dependency is not
> re-exported by `@elyra/canvas` and is not present in the harness.

When this callback is registered it is called once per expression editor mount
(and any time the editor is rebuilt due to a visibility-state transition).
...
**Return value:**
Return a CodeMirror [`LanguageSupport`](https://codemirror.net/docs/ref/#language.LanguageSupport)
instance to override the built-in language resolution entirely, or `null` / `undefined`
to fall back to the built-in behaviour.

```js
import { LanguageSupport, StreamLanguage } from "@codemirror/language";

// Minimal custom tokenizer — replace with a real grammar for production use
const juelLang = StreamLanguage.define({ token: (stream) => { stream.next(); return null; } });

expressionLanguageHandler(propertyId, language) {
  if (language === "juel") {
    return new LanguageSupport(juelLang);
  }
  return null; // fall back to built-in for all other controls
}
```

> Note: `expressionVariablesHandler` completions are always appended on top of whatever
> `LanguageSupport` provides - this is handled internally and does not need to be
> replicated in the handler.
