...
### buttonIconHandler
 This handler is used to display the Carbon icon in the custom table button and action buttons based on the `type` field of the data. This buttonIconHandler expects a Carbon Icon jsx object as the return value from the callback. The buttonIconHandler provides the following `type`:
...
- data: an object that consists of
    ...
    - data: consist of entire action object.
