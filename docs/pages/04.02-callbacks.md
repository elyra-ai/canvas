...
### propertyIconHandler()
Called when a user wants to pass in a specific object to a dropdown menu. The propertyIconHandler expects a Carbon jsx object as the return value from the callback. This is used to display the Carbon icon in the dropdown menu. The propertyIconHandler provides the following data:
