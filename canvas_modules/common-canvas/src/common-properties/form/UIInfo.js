...
export class UIInfo {
	constructor(elements) {
		...
		if (elements.resourceKey) {
			this.resourceKey = elements.resourceKey;
		}
		if (elements.uiHints) {
			this.uiHints = elements.uiHints;
		}
	}
	...
	uiHintOrElse(attr, defaultValue) {
		if (this.uiHints) {
			const hint = _.propertyOf(this.uiHints)(attr);
			if (hint) {
				return hint;
			}
			return defaultValue;
	 ...
	 */
	orientation() {
		return this.uiHintOrElse("orientation");
	}
