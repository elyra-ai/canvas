...
function _makeUIItem(parameterMetadata, actionMetadata, group, structureMetadata, l10nProvider, invertColor = false) {
	...
	case GroupType.CONTROLS:
		return UIItem.makePanel(new ControlPanel(groupName, PanelType.GENERAL, groupClassName, nestedPanel,
			_makeControls(parameterMetadata, actionMetadata, group, structureMetadata, l10nProvider, invertColor)));
	...
	case GroupType.SUB_TABS: {
		...
		if (Array.isArray(group.subGroups)) {
			group.subGroups.forEach(function(subGroup) {
				...
				groupItem = _makeUIItem(parameterMetadata, actionMetadata, subGroup, structureMetadata, l10nProvider, invertColor);
 ...
 */
function _makeControls(parameterMetadata, actionMetadata, group, structureMetadata, l10nProvider, invertColor = false) {
	const uiItems = [];
	...
	group.parameterNames().forEach(function(paramName) {
		...
		if (!(group instanceof StructureDef) || (group instanceof StructureDef && prop.isSubPanelEdit())) {
			const ctrl = _makeControl(parameterMetadata, paramName, group, structureDef, l10nProvider, actionMetadata, structureMetadata, null, invertColor);
	...
	if (Array.isArray(group.subGroups)) {
		group.subGroups.forEach(function(subGroup) {
			if (!_hasPanelBeenInserted(panelInsertedFor, subGroup.dependsOn)) {
				const uiItem = _makeUIItem(parameterMetadata, actionMetadata, subGroup, structureMetadata, l10nProvider, invertColor);
				uiItems.push(uiItem);
			}
		});
	}
 ...
 */
function _makeControl(parameterMetadata, paramName, group, structureDefinition, l10nProvider, actionMetadata, structureMetadata, subControl, invertColor = false) {
