...
export default class SvgCanvasNodes {
	...
	getNodeLabelPosX(node) {
		...
		return node.layout.labelAlign === "center" ? x - (node.layout.labelMaxWidth / 2) : x;
	}
	...
	getNodeLabelWidth(node) {
		...
		return node.layout.labelMaxWidth;
	}
	...
	getNodeLabelHoverWidth(node) {
		if (node.layout.labelAlign === "center") {
			return node.layout.labelMaxWidth + 500;
		}
		return node.layout.labelMaxWidth + 40;
	}
