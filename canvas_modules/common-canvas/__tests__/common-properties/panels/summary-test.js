...
describe("summary panel renders error/warning status correctly", () => {
	...
	it("should show error icon in summary when both error and warning messages exist", () => {
		...
		expect(alertCategory.find("button.bx--accordion__heading").text()).to.equal("Alerts (2)");
		...
		expect(tableCategory.find("button.bx--accordion__heading").text()).to.equal("Structure List Table (2)");
		let summary = tableCategory.find("div.properties-summary-link-container");
		...
		expect(alertCategory.find("button.bx--accordion__heading").text()).to.equal("Alerts (1)");
		...
		expect(tableCategory.find("button.bx--accordion__heading").text()).to.equal("Structure List Table (1)");
		summary = tableCategory.find("div.properties-summary-link-container");
		expect(summary.find("svg.warning")).to.have.length(1);
	});
});
