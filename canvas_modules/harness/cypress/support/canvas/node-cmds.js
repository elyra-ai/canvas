...
function getNodeGrpSelector() {
	...
	const selector = `div[role='application'] > svg > g > g > g[data-id^='node_grp_${inst}']`;
	return selector;
}

function getNodeGrpSelectorInSubFlow() {
	const inst = document.extraCanvas === true ? "1" : "0";
	const selector = `div[role='application'] > svg > g > g > g > svg > g > g > g[data-id^='node_grp_${inst}']`;
	return selector;
}
...
function getNodeGrpSelectorInSupernode(supernodeId) {
	const inst = document.extraCanvas === true ? "1" : "0";
	const selector =
	`div[role='application'] > svg > g > g > g[data-id='node_grp_${inst}_${supernodeId}'] > svg > g > g > g[data-id^='node_grp_${inst}']`;
	return selector;
}
...
// Alias for verifyPortConnectFromArrowExists
Cypress.Commands.add("verifyPortHasConnectFromArrow", (nodeLabel) => {
	cy.verifyPortConnectFromArrowExists(nodeLabel);
});
...
// Alias for verifyPortConnectFromArrowDoesNotExist
Cypress.Commands.add("verifyPortDoesNotHaveConnectFromArrow", (nodeLabel) => {
	cy.verifyPortConnectFromArrowDoesNotExist(nodeLabel);
});
