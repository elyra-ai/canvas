...
1. By using the following `CanvasController` methods:
    ...
    ```
    See the [CanvasController API documentation](/2.4-Canvas-Controller-API) for more details.

2. Node decorations can be specified in the [nodeLayout object](/2.1-Config-Objects#enablenodelayout) in the canvas config. Decorations specified in this way are applied to all nodes on the canvas. 
...
4. Decorations can be returned in the layout information returned from the [layoutHandler](/2.2-Common-Canvas-callbacks#layouthandler)  CommonCanvas callback method.
...
#### **hotspot**
A Boolean. It defaults to false. When set to true the decoration becomes clickable and when it is clicked the [decorationCallbackHandler](/2.2-Common-Canvas-callbacks#decorationactionhandler) is called with the ID of the decoration passed as a parameter.   
